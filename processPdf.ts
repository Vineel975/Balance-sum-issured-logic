  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (tariffMatch.tariffClarificationNote !== undefined) {
        ...
        // ── BSI: fetch live Balance Sum Insured from McarePlus DB ────────
        // Non-blocking — a failure here never crashes the pipeline.
        // When BSI is available it:
        //   1. Replaces AI-extracted sumInsuredAmount with the real DB value
        //      (fixes PSU bracket: ≤5L vs >5L)
        //   2. Caps finalInsurerPayable at EffectiveBalance so we never
        //      recommend paying more than the member has available
        if (job?.claimId) {
          try {
            const bsiData = await getBalanceSumInsured(job.claimId);

            if (bsiData) {
              // Store on analysis so the UI can render the BSI table
              (result.analysis as Record<string, unknown>).bsiData = bsiData;

              const baseSI = bsiData.Suminsured?.[0];

              // Override AI-extracted SI with the real DB value
              if (baseSI?.Suminsured && result.analysis.policyRuleContext) {
                result.analysis.policyRuleContext.sumInsuredAmount =
                  baseSI.Suminsured;
              }

              // Apply EffectiveBalance as a hard cap
              const effBal = baseSI?.EffectiveBalance;
              if (
                typeof effBal === "number" &&
                effBal >= 0 &&
                typeof result.analysis.finalInsurerPayable === "number"
              ) {
                const aiPayable = result.analysis.finalInsurerPayable;
                const capped    = Math.min(aiPayable, effBal);
                result.analysis.finalInsurerPayable = capped;

                if (capped < aiPayable) {
                  result.analysis.finalInsurerPayableNotes =
                    (result.analysis.finalInsurerPayableNotes ?? "") +
                    ` SI balance cap: effective balance INR ${effBal.toFixed(2)}` +
                    ` reduced payable from INR ${aiPayable.toFixed(2)}` +
                    ` to INR ${capped.toFixed(2)}.`;
                }
              }
              ...
              await ctx.runMutation(api.processing.addLog, {
                jobId: args.jobId,
                message: formatLogMessage(
                  `[BSI] claimId=${job.claimId}` +
                  ` SI=${baseSI?.Suminsured ?? "?"}` +
                  ` EffBal=${baseSI?.EffectiveBalance ?? "?"}` +
                  ` Utilized=${baseSI?.Utilized ?? "?"}`,
                ),
              });
            } else {
              await ctx.runMutation(api.processing.addLog, {
                jobId: args.jobId,
                message: formatLogMessage(
                  `[BSI] Not available for claimId=${job.claimId} — AI payable used as-is`,
                ),
              });
            }
          } catch (bsiErr) {
            await ctx.runMutation(api.processing.addLog, {
              jobId: args.jobId,
              message: formatLogMessage(
                `[BSI] Error: ${bsiErr instanceof Error ? bsiErr.message : String(bsiErr)}`,
              ),
            });
          }
        }
