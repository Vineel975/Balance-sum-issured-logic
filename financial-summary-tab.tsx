...
interface FinancialSummaryTabProps {
  ...
  /** Live BSI from McarePlus DB — stored by Convex during processing */
...
export function FinancialSummaryTab({
  ...
  const bsiBaseSI =
    ...
    bsiData?.Suminsured?.[0] ??
    null;
  const bsiEffectiveBalance: number | null =
    typeof bsiBaseSI?.EffectiveBalance === "number"
      ? bsiBaseSI.EffectiveBalance
      : null;
  ...
  const bsiCapApplied: boolean =
    bsiCappedPayable !== null &&
    totalAmountApproved !== null &&
  ...
  const goToLensTypePage = () => {
    if (lensTypePageNumber && onTariffAmountClick) onTariffAmountClick(lensTypePageNumber);
  };
  ...
  const goToHospitalPage = () => {
    if (hospitalBillPageNumber && onHospitalAmountClick) onHospitalAmountClick(hospitalBillPageNumber);
  };
  const goToTariffPage = () => {
    if (tariffPageNumber && onTariffAmountClick) onTariffAmountClick(tariffPageNumber);
  };
            ...
            <CardContent className="pt-4">
              ...
              {!bsiData && (
                <div className="text-xs text-slate-400">
                  BSI not available — check DB connection or claimId in Convex logs.
                </div>
              )}
                        ...
                        {bsiData.Suminsured.map((row, idx) => (
                          ...
                          >
                            <td className="border-t border-slate-100 px-2 py-1.5">
                              {row.BPSIID}
                              {row.SICategery === 69 && (
                                <span className="ml-1 text-[9px] text-slate-400">base</span>
                              )}
                            </td>
